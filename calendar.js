// 🔄 Fetch Anime Releases (no auth needed)
...
// 🔒 Fetch Manga Releases from Cloudflare Worker
async function fetchMangaReleases() {
  try {
    const res = await fetch("https://auth-manga-dex.keshavkdas23.workers.dev/");
    const json = await res.json();

    console.log("📦 Manga response:", json);

    if (!json || !json.data || !Array.isArray(json.data)) {
      console.error("❌ Invalid manga format:", json);
      return [];
    }
...
async function buildCalendar() {
  ...
  try {
    ...
    console.log("📦 Combined releases:", releases);
  } catch (e) {
    console.error("❌ Error combining data:", e);
    releases = [];
  }
